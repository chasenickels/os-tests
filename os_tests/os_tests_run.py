...
from os_tests.libs.utils_lib import (
    filter_case_doc,
    get_cfg,
    init_args,
    init_provider,
...
def main():
    ...
    images_check = {
        "azure": "test_azure_image",
        "gcp": "test_gcp_image",
        "kvm": "test_rhel_guest_image",
        "rhel-kvm": "test_rhel_guest_image",
    }

    if params.get("image") is not None:
        if params.get("image") not in images_check.keys():
            log.info(
                "only {} image check supported for now".format(images_check.keys())
            )
            sys.exit(0)
        test_patterns = "{}{}{}".format(
            test_patterns or "",
            test_patterns and "," or "",
            images_check.get(params.get("image")),
        )
    else:
    ...
    log.info(
        "Run in mode: is_listcase:{} test_patterns:{} skip_patterns:{}".format(
            params.get("is_listcase"), test_patterns, skip_patterns
        )
    )
                ...
                try:
                    for case in ts2._tests:
                        ...
                        if filter_case_doc(
                            ...
                            filter_field=params.get("filter_by"),
                            strict=params.get("is_strict"),
                            verify_doc=params.get("verifydoc"),
                        ):
    ...
    if test_patterns and "case_name" in params.get("filter_by"):
        ...
        for pattern in test_patterns.split(","):
            for case in tests_list:
                if (
                    re.match(".*{}.*".format(pattern), case.id(), re.IGNORECASE)
                    and case not in sorted_tests
                ):
        ...
        log.info(
            "\n".join(
                [
                    "{} - {}/{}".format(
                        case_name,
                        case_name_list.index(case_name) + 1,
                        len(case_name_list),
                    )
    ...
    for res in chain(vms, disks, nics):
        if params.get("no_cleanup"):
            log.info(
                "skipped resource cleanup because --no-cleanup found, please release resources manually"
            )
