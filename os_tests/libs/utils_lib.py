...
def init_args():
    ...
    parser.add_argument(
        ...
        dest="is_listcase",
        action="store_true",
        help="list supported cases without run",
        required=False,
    )
    parser.add_argument(
        "-p",
        ...
        default=None,
        action="store",
        help="filter case by name and run it in specified order, add --strict for matching exactly",
        required=False,
    )
    parser.add_argument(
        "--strict",
        dest="is_strict",
        action="store_true",
        help="match exactly if -p or -s specified",
        required=False,
    )
    parser.add_argument(
        "-s",
        ...
        default=None,
        action="store",
        help="skip cases, add --strict for skipping exactly",
        required=False,
    )
    parser.add_argument(
        "--verifydoc",
        dest="verifydoc",
        action="store_true",
        help="verify or show case doc only",
        required=False,
    )
    parser.add_argument(
        "--dumpdoc",
    ...
    parser.add_argument(
        ...
        default=None,
        action="store",
        help="run tests on remote nodes",
        required=False,
    )
    parser.add_argument(
        "--port",
    ...
    parser.add_argument(
        ...
        default=None,
        action="store",
        help="user to login to remote node",
        required=False,
    )
    parser.add_argument(
        "--password",
        ...
        default=None,
        action="store",
        help="password to login to remote node",
        required=False,
    )
    parser.add_argument(
        "--keyfile",
        ...
        default=None,
        action="store",
        help="keyfile to login to remote node",
        required=False,
    )
    parser.add_argument(
        "--result",
        ...
        default=None,
        action="store",
        help="save result to specific directory",
        required=False,
    )
    parser.add_argument(
        "--image",
        ...
        default=None,
        action="store",
        help="specify azure to run azure image check only",
        required=False,
    )
    parser.add_argument(
        "--params_profile",
        ...
        default=None,
        action="store",
        help="params in yaml file, you can also put platform_profile content in this file",
        required=False,
    )
    parser.add_argument(
        "--platform_profile",
        dest="platform_profile",
        default=None,
        action="store",
        help="specify platform profile if enable os-tests provison vms self, [alicloud, aws, azure, gcp, openstack, libvirt, nutanix, openshift, openstack]",
        required=False,
    )
    parser.add_argument(
        "--no-cleanup",
        dest="no_cleanup",
        action="store_true",
        help="debug purpose, skip cleanup phase at exit, do not use it in normal test",
        required=False,
    )
    parser.add_argument(
        "--proxy_url",
        ...
        default=None,
        action="store",
        help="specify it if pkg/repo url is internal only, format IP:PORT",
        required=False,
    )
    parser.add_argument(
        "--case_setup",
        ...
        default=None,
        action="store",
        help="fips_enable,fips_disable,debugkernel_enable,debugkernel_disable,commands or a bash file before running into case steps,can add timeout:XXX if default 600 is not enough for operation done",
        required=False,
    )
    parser.add_argument(
        "--case_post",
        ...
        default=None,
        action="store",
        help="collect_kmemleak, other args are similar as case_setup, usually for collecting temporary debug information, can add timeout:XXX if default 600 is not enough for operation done",
        required=False,
    )
    parser.add_argument(
        "--subscription_username",
        dest="subscription_username",
        default=None,
        action="store",
        help="username for RHSM register",
        required=False,
    )
    parser.add_argument(
        "--subscription_password",
        dest="subscription_password",
        default=None,
        action="store",
        help="password for RHSM register",
        required=False,
    )
    parser.add_argument(
        "--dnf_repo_url",
        ...
        default=None,
        action="store",
        help='specify it when using customized repos for dnf update, seperated by ","',
        required=False,
    )
    parser.add_argument(
        "--leapp_target_repo_url",
        dest="leapp_target_repo_url",
        default=None,
        action="store",
        help='specify it when leapp upgrade via custom repo, seperated by ","',
        required=False,
    )
    parser.add_argument(
        "--pkg_repo_url",
        ...
        default=None,
        action="store",
        help='specify it when using customized repos for package installation, seperated by ","',
        required=False,
    )
    parser.add_argument(
        "--pkgs",
        ...
        default=None,
        action="store",
        help='specify packages names (and version if needed) you want to install, seperated by ","',
        required=False,
    )
    parser.add_argument(
        "--target_version",
        ...
        default=None,
        action="store",
        help="specify the target version you want to upgrade to, e.g., 9.4",
        required=False,
    )
...
def init_provider(params=None):
    ...
    if not provider:
        ...
        LOG.info(
            "{} not in supported platforms:{}".format(provider, supported_platforms)
        )
        sys.exit(1)

    if "aws" in provider:
        from .resources_aws import EC2NIC, EC2VM, EC2Volume
        ...
        if params.get("instance_type") and "," in params.get("instance_type"):
            vms[1].instance_type = (
                params.get("instance_type").split(",")[-1]
                or params.get("instance_type").split(",")[0]
            )
...
def init_provider_from_guest(test_instance):
    ...
    if os.getenv("INFRA_PROVIDER"):
        test_instance.log.info(
            "already set provider to {}".format(os.getenv("INFRA_PROVIDER"))
        )
        return True
    provider = "UNKNOW"
...
def update_cfgs(
    base_cfg={}, new_cfg={}, keep_base=False, only_update_exists_keys=False
):
    """
...
def init_connection(
    test_instance, timeout=600, interval=10, rmt_node=None, vm=None, retry=3
):
    if (
        not test_instance.params["remote_node"]
        and not rmt_node
        and not vm
        and not test_instance.vm
    ):
        return False
    new_vm_ip = None
    is_master_vm = False
    if test_instance.vm:
        ...
        if new_vm_ip != test_instance.params["remote_node"]:
            ...
            test_instance.log.info(
                "set default remote_node to new address {}".format(
                    test_instance.params["remote_node"]
                )
            )
            is_master_vm = True
    test_instance.log.info(
        "Current IP bucket:{}".format(test_instance.params["remote_nodes"])
    )
    ...
    if vm:
        ...
        if vm.dead_count > 4:
            test_instance.fail(
                "cannot connect to vm over 4 times, skip retry to connect it"
            )
    if not rmt_node:
    ...
    if not ssh.ssh_client:
        if vm:
            try:
                ...
                test_instance.log.info(
                    "{} not implement this func: get_console_log".format(vm.provider)
                )
            vm.dead_count += 1
            test_instance.log.info("vm dead times:{}".format(vm.dead_count))
            if vm.dead_count == 2:
                test_instance.log.info(
                    "vm cannot connect in {} times, restart it".format(vm.dead_count)
                )
                vm.stop()
                vm.start()
            if vm.dead_count == 3:
                test_instance.log.info(
                    "vm cannot connect in {} times, re-create it".format(vm.dead_count)
                )
                vm.delete()
                vm.create()
...
def init_case(test_instance):
    ...
    for key in test_instance.params.keys():
        if (
            key in ["password", "subscription_username", "subscription_password"]
            or "password" in key
        ):
            test_instance.log.info("key:{}, val:*******".format(key))
        else:
            test_instance.log.info(
                "key:{}, val:{}".format(key, test_instance.params[key])
            )
    test_instance.log.info("-" * 80)
    test_instance.ssh_timeout = 180
    test_instance.distro = init_distro(test_instance)
    test_instance.file = File(test_instance)
...
""".format(
        ...
        if hasattr(test_instance.vm, "get_state") and "stopping" in str(
            test_instance.vm.get_state()
        ):
    ...
    if test_instance.is_rmt:
        test_instance.log.info(
            "ssh connection timeout:{}".format(test_instance.ssh_timeout)
        )
        ...
        if test_instance.SSH:
            ...
            if not test_instance.SSH.ssh_client:
                test_instance.fail(
                    "Cannot make ssh connection to remote, please check!"
                )
    node_info = "{}/node_info".format(attachment_dir)
    node_info_data = {}
    if not os.path.exists(node_info):
        test_instance.log.info("retrive node info.")
        node_info_data["release_name"] = get_os_release_info(
            test_instance, field="NAME"
        ).rstrip("\n| ")
        node_info_data["release_version"] = get_os_release_info(
            test_instance, field="VERSION"
        ).rstrip("\n| ")
        node_info_data["kernel_version"] = run_cmd(test_instance, "uname -r").rstrip(
            "\n| "
        )
        node_info_data["product_name"] = run_cmd(
            test_instance, "cat /sys/devices/virtual/dmi/id/product_name"
        ).rstrip("\n| ")
        node_info_data["sys_vendor"] = run_cmd(
            test_instance, "cat /sys/devices/virtual/dmi/id/sys_vendor"
        ).rstrip("\n| ")
...
def finish_case(test_instance):
    ...
    Arguments:
        test_instance {Test instance} -- unittest.TestCase instance
    """
    for case, reason in chain(
        test_instance._outcome.result.failures, test_instance._outcome.result.errors
    ):
...
def extra_step_parser(test_instance, extra_steps=None):
    if not extra_steps:
        test_instance.log.info(
            "no extra steps detected, you can try case_setup or case_setup to inject more operations before or after case."
        )
        return True
    ...
    for extra_case_setup in extra_steps.split(","):
        if "timeout:" in extra_case_setup:
            ...
            test_instance.log.info(
                "extra case exec timeout detected, use it:{}".format(exe_timout)
            )
            break
    for extra_case_setup in extra_steps.split(","):
        ...
        if cmdfile:
            ret = run_cmd(
                test_instance,
                cmd="sudo ls {}".format(cmdfile),
                ret_status=True,
                msg="guess {} is a file".format(cmdfile),
            )
            if ret == 0:
                cmdfile_exists = True
        cmd_options = ""
        ...
        if os.path.isfile(cmdfile):
            ...
            run_cmd(
                test_instance, cmd="cat {}".format(rmt_file), msg="show file content"
            )
            run_cmd(
                test_instance,
                cmd="sudo chmod 777 {}".format(rmt_file),
                msg="add execute permission",
            )
            run_cmd(
                test_instance,
                cmd="sudo {} {}".format(rmt_file, cmd_options),
                timeout=exe_timout,
                msg="run the script",
            )
        else:
            ...
            if not is_func:
                run_cmd(
                    test_instance,
                    cmd=extra_case_setup,
                    timeout=exe_timout,
                    msg="run the {} content as command".format(extra_case_setup),
                )
...
def run_cmd(
    ...
    try:
        if test_instance.is_rmt:
            ...
            status, output = SSH.remote_excute(
                cmd,
                timeout,
                is_log_cmd,
                redirect_stdout=rmt_redirect_stdout,
                redirect_stderr=rmt_redirect_stderr,
                rmt_get_pty=rmt_get_pty,
            )
        else:
            status, output = run_cmd_local(
                cmd=cmd, timeout=timeout, is_log_cmd=is_log_cmd, log=test_instance.log
            )
    ...
    if exception_hit:
        test_cmd = "uname -a"
        test_instance.log.info(
            "Test system is alive via cmd:{}. If still fail, check no hang or panic happens.".format(
                test_cmd
            )
        )
        try:
            if test_instance.is_rmt:
                if "Key-exchange timed out" in run_err:
                    test_instance.log.info(
                        "reconnect to remote because it acheived certain number of packets or bytes sent or received using this session"
                    )
        ...
        for vm in test_instance.vms:
            if vm.exists() and rmt_node == vm.floating_ip:
                try:
                    ...
                    test_instance.log.info(
                        "{} not implement this func: get_console_log".format(
                            test_instance.vm.provider
                        )
                    )
    ...
    if expect_ret is not None:
        test_instance.assertEqual(
            status, expect_ret, msg="ret is %s, expected is %s" % (status, expect_ret)
        )
    if expect_not_ret is not None:
        test_instance.assertNotEqual(
            status,
            expect_not_ret,
            msg="ret is %s, expected not ret is %s" % (status, expect_not_ret),
        )
    if expect_kw is not None:
        for key_word in expect_kw.split(","):
            find_list = re.findall(
                "{}".format(".*" in key_word and key_word or ".*{}.*".format(key_word)),
                output,
            )
            if find_list:
                test_instance.log.info(
                    'expected "{}" found in "{}"'.format(key_word, "".join(find_list))
                )
            else:
                if output is not None and output.count("\n") > 5:
                    test_instance.fail(
                        'expected "{}" not found in output(check debug log as too many lines)'.format(
                            key_word
                        )
                    )
                else:
                    test_instance.fail(
                        'expected "{}" not found in "{}"'.format(key_word, output)
                    )
    if expect_not_kw is not None:
        for key_word in expect_not_kw.split(","):
            ...
            if not find_list:
                test_instance.log.info(
                    'Unexpected "{}" not found in output'.format(key_word)
                )
            else:
                if output is not None and output.count("\n") > 5:
                    test_instance.fail(
                        'Unexpected "{}" found in {}'.format(
                            key_word, "".join(find_list)
                        )
                    )
                else:
                    test_instance.fail(
                        'Unexpected "{}" found in "{}"'.format(key_word, output)
                    )
    if expect_output is not None:
        test_instance.assertEqual(
            expect_output,
            output.rstrip("\n"),
            msg="exactly expected %s" % (expect_output),
        )
    if cancel_kw is not None:
        ...
        if cancel_yes:
            test_instance.skipTest(
                "'%s' not found, cancel case. msg:%s" % (cancel_kw, msg)
            )
    if cancel_not_kw is not None:
        for key_word in cancel_not_kw.split(","):
            if key_word in output:
                test_instance.skipTest(
                    "'%s' found, cancel case. msg:%s" % (key_word, msg)
                )
    if cancel_ret is not None:
        ...
        if cancel_yes:
            test_instance.skipTest(
                "expected ret code '{}' not match ret '{}', cancel case. msg:{}".format(
                    cancel_ret, status, msg
                )
            )
    if cancel_not_ret is not None:
        for ret in cancel_not_ret.split(","):
            if int(ret) == int(status):
                test_instance.skipTest(
                    "skip ret code '%s' found act ret '%s' cancel case. msg:%s"
                    % (ret, status, msg)
                )
    if ret_status:
        return status
...
def getboottime(test_instance):
    ...
    if "min" in boot_time:
        ...
        boot_time_sec = (
            int(boot_time_min) * 60 + decimal.Decimal(boot_time_sec).to_integral()
        )
    test_instance.log.info("Boot time is {}(s)".format(boot_time_sec))
...
def is_sev_enabled(test_instance):
    ...
    if test_instance.vm.provider == "google":
        ...
        cmd = "curl -s -H 'Metadata-Flavor: Google' \
            'http://metadata/computeMetadata/v1/instance/service-accounts/default/identity?audience={}&format=full'".format(
            audience
        )
...
def is_aws(test_instance, action=None):
    ...
    """
    if is_pkg_installed(
        test_instance, pkg_name="virt-what", cancel_case=False, is_install=False
    ):
        output = run_cmd(
            test_instance,
            "sudo bash -c 'cat /sys/devices/virtual/dmi/id/bios_*; virt-what'",
            expect_ret=0,
        )
    else:
        output = run_cmd(
            test_instance, "sudo cat /sys/devices/virtual/dmi/id/bios_*", expect_ret=0
        )
...
def is_ali(test_instance, action=None):
    ...
    if is_pkg_installed(
        ...
        output = run_cmd(
            test_instance,
            "sudo bash -c 'cat /sys/devices/virtual/dmi/id/product_*; virt-what'",
            expect_ret=0,
        )
    else:
        output = run_cmd(
            test_instance,
            "sudo cat /sys/devices/virtual/dmi/id/product_*",
            expect_ret=0,
        )
...
def is_metal(test_instance, action=None):
    ...
    if "x86_64" in output_lscpu and "Hypervisor" not in output_lscpu:
        ...
        if action == "cancel":
            test_instance.skipTest("Cancel it in non metal system.")
        return False
    output_dmesg = run_cmd(
        test_instance, "sudo dmesg", expect_ret=0, is_log_output=False
    )
...
def is_cmd_exist(
    test_instance, cmd=None, is_install=True, cancel_case=False, rmt_node=None, vm=None
):
    """
    ...
    out = run_cmd(
        test_instance, "sudo yum info {}".format(pkg_list[0]), rmt_node=rmt_node, vm=vm
    )
    ...
    run_cmd(
        test_instance,
        "sudo yum install -y {}".format(pkg_name),
        expect_ret=0,
        timeout=720,
        rmt_node=rmt_node,
        vm=vm,
...
def is_pkg_installed(
    ...
    Arguments:
        ...
        is_install {bool} -- try to install it or not
    """
    cmd = test_instance.distro.is_pkg_installed(pkg_name)
    ret = run_cmd(
        test_instance, cmd, timeout=1200, ret_status=True, rmt_node=rmt_node, vm=vm
    )
    if ret == 0:
        return True
    else:
        ...
        if is_install:
            cmd = test_instance.distro.install_cmd(pkg_name)
            ret = run_cmd(
                test_instance,
                cmd,
                ret_status=True,
                msg="try to install it",
                timeout=timeout,
                rmt_node=rmt_node,
                vm=vm,
            )
            if ret == 0:
                return True
        if cancel_case:
            test_instance.skipTest("Unable to install {}".format(pkg_name))
        return False


def pkg_install(
    test_instance, pkg_name=None, pkg_url=None, force=False, rmt_node=None, vm=None
):
    """
    ...
    if not is_pkg_installed(
        ...
        if pkg_url is not None:
            ...
            cmd = test_instance.distro.install_cmd(pkg_url)
        else:
        ...
        if pkg_url is not None:
            ...
            cmd = test_instance.distro.reinstall_cmd(pkg_url)
        else:
            test_instance.log.info("Reinstall {} from default repo".format(pkg_name))
            cmd = test_instance.distro.reinstall_cmd(pkg_name)
        run_cmd(test_instance, cmd, timeout=1200, rmt_node=rmt_node, vm=vm)

    if (
        not is_pkg_installed(
            test_instance,
            pkg_name=pkg_name,
            cancel_case=False,
            is_install=False,
        ...
        and pkg_url is not None
        and force
    ):
        test_instance.log.info("Install without dependences!")
        cmd = test_instance.distro.install_nodeps(pkg_url)
        if force:
            cmd = cmd + " --force"
...
def is_rhsm_registered(
    test_instance, cancel_case=False, timeout=600, rmt_node=None, vm=None
):
    """
    ...
    out = run_cmd(
        test_instance,
        cmd,
        msg="try to check subscription status",
        rmt_node=rmt_node,
        vm=vm,
    )
    if "Red Hat Enterprise Linux" in out or "Simple Content Access" in out:
        ...
        if cancel_case:
            test_instance.skipTest("Unable to register")
        return False
...
def enable_auto_registration(
    ...
    for cmd in cmds_autoreg:
        run_cmd(
            test_instance, cmd, timeout=timeout, expect_ret=0, rmt_node=rmt_node, vm=vm
        )
    ...
    while True:
        ...
        if end_time - start_time > timeout:
            test_instance.fail(
                "timeout({}s) to wait auto subscription registered completed".format(
                    timeout
                )
            )
        test_instance.log.info(
            "wait {}s and try to check again, timeout {}s".format(interval, timeout)
        )
        time.sleep(interval)
...
def rhsm_register(
    ...
    """
    if not is_rhsm_registered(
        test_instance, cancel_case=False, rmt_node=rmt_node, vm=vm
    ):
        if is_aws(test_instance):
...
def check_log(
    test_instance,
    log_keyword,
    log_cmd="journalctl -b 0",
    expect_ret=None,
    expect_not_ret=None,
    match_word_exact=False,
    cursor=None,
    skip_words=None,
    rmt_redirect_stdout=False,
    rmt_redirect_stderr=False,
    ...
    for keyword in log_keyword.split(","):
        ...
        if ret:
            ...
            test_instance.log.info(
                "No unexpected {} in {} log!".format(keyword, check_cmd)
            )
...
def find_word(
    ...
    if not baseline_dict:
        ...
        with open(baseline_file, "r") as fh:
            test_instance.log.info(
                "Loading baseline data file from {}".format(baseline_file)
            )
        ...
        for key in baseline_dict.keys():
            if baseline_dict[key].get("cases"):
                if case in baseline_dict[key].get("cases"):
                    ...
                    msg_ids.append(key)
        test_instance.log.info(
            "fetched rules from {} matching case:{}".format(msg_ids, case)
        )
        kw_from_case = True
        if not log_keywords:
    ...
    for kw in log_keywords:
        tmp_list = re.findall(
            ".*" in kw and kw or ".*{}.*".format(kw), check_str, flags=re.I
        )
        if len(tmp_list) == 0:
        ...
        if len(tmp_list) == 0:
            test_instance.log.info(
                "No {} found after skipped {}!".format(log_keyword, skip_words)
            )
            continue
    ...
    for line1 in set(all_items):
        ...
        for basekey in baseline_dict:
            if (
                case
                and baseline_dict[basekey].get("cases")
                and case not in baseline_dict[basekey].get("cases")
            ):
                continue
        ...
        if not found_it:
            ...
            for basekey in baseline_dict:
                if (
                    case
                    and baseline_dict[basekey].get("cases")
                    and case not in baseline_dict[key].get("cases")
                ):
                    continue
                line1_tmp = line1
                line2_tmp = baseline_dict[basekey]["content"]
                line1_tmp, line2_tmp = clean_sentence(
                    test_instance, line1_tmp, line2_tmp
                )
                seq = difflib.SequenceMatcher(None, a=line1_tmp, b=line2_tmp)
                same_rate = seq.ratio() * 100
                if same_rate > pass_rate:
                    test_instance.log.info(
                        "content similar rate:{} over {} baseline:{}".format(
                            same_rate, pass_rate, basekey
                        )
                    )
                    matched_msg = basekey
                    found_it = True
        ...
        if found_it:
            ...
            if baseline_dict[basekey]["status"] != "active":
                test_instance.log.info(
                    "Found a similar log {} matched in baseline. But it is not active, please check manually".format(
                        basekey
                    )
                )
                new_fail_found = True
            ...
            if trigger and re.search(trigger, check_str, flags=re.I):
                test_instance.log.info(
                    "Guess it is expected because trigger keywords found '{}'".format(
                        trigger
                    )
                )
                ...
                test_instance.log.info(
                    "Guess it is unexpected because trigger keywords not found '{}'".format(
                        trigger
                    )
                )
                new_fail_found = True
...
def set_service(
    ...
    if not enable_it:
        cmd_dict = [
            {
                "cmd": "sudo systemctl stop {}".format(service),
                "msg": "stop {}".format(service),
            },
            {
                "cmd": "sudo systemctl disable {}".format(service),
                "msg": "disable {}".format(service),
            },
            {
                "cmd": "sudo systemctl mask {}".format(service),
                "msg": "mask {} to prevent others bring it up".format(service),
            },
        ]
    else:
        cmd_dict = [
            {
                "cmd": "sudo systemctl unmask {}".format(service),
                "msg": "unmask {}".format(service),
            },
            {
                "cmd": "sudo systemctl enable --now {}".format(service),
                "msg": "enable and start {}".format(service),
            },
        ]
    for cmd in cmd_dict:
        if check_ret:
...
def wait_for(
    ret=None, not_ret=None, ck_ret=False, ck_not_ret=False, timeout=60, interval=1
):
    """
...
def iterate_timeout(timeout, message, wait=2):
    ...
    while (timeout is None) or (time.time() < start + timeout):
        ...
        yield count
        logging.debug(
            "Have been waiting for %ss/%ss." % (int(time.time() - start), timeout)
        )
        time.sleep(wait)
    ...
    if "No such file or directory" not in core_files:
        ...
        for core_file in core_files.split("\n"):
            ...
            if test_instance.params.get("remote_node") is not None:
                test_instance.log.info(
                    "retrive {} from remote to {}".format(
                        core_file, test_instance.log_dir
                    )
                )
                test_instance.SSH.get_file(
                    rmt_file=core_file,
                    local_file="{}/attachments/{}".format(
                        test_instance.log_dir, os.path.basename(core_file)
                    ),
                )
            else:
                cmd = "cp {} {}/attachments/{}".format(
                    core_file, test_instance.log_dir, os.path.basename(core_file)
                )
                run_cmd(
                    test_instance,
                    cmd,
                    msg="save {} to {}".format(core_file, test_instance.log_dir),
                )
        run_cmd(
            test_instance,
            "sudo rm -rf /var/lib/systemd/coredump/core*",
            msg="clean up core files",
        )
    ...
    for disk in disk_list:
        ...
        for mount_disk in mount_disks:
            if disk in mount_disk:
                ...
                break
        if not disk_in_use:
            cmd = "sudo wipefs -a /dev/{}".format(
                disk
            )  # comment this line for bz2074486
            ret = run_cmd(
                test_instance,
                cmd,
                ret_status=True,
                timeout=1800,
                msg="test can clean fs on {}".format(disk),
            )
            if ret == 0:
                test_disk = disk
                break
...
def debugkernel_enable(test_instance=None):
    ...
    if test_instance.vm and is_ahv(test_instance) and test_instance.vm.is_secure_boot:
        test_instance.log.info(
            """Red Hat Insights error "sed: can't read /sys/kernel/debug/sched_features: \
    ...
    if "debug" in kernel_version:
        ...
        if "kmemleak=on" in cmdline:
            ...
            run_cmd(
                ...
                cmd,
                expect_ret=0,
                msg='Add "kmemleak=on" to /proc/cmdline',
            )
    else:
        ...
        run_cmd(
            ...
            expect_ret=0,
            msg="check if kernel-debug is installed",
        )
...
def debugkernel_disable(test_instance=None):
    ...
    if "debug" not in kernel_version:
        ...
        if "kmemleak=on" not in cmdline:
            ...
            run_cmd(
                ...
                cmd,
                expect_ret=0,
                msg='Remove "kmemleak=on" from /proc/cmdline',
            )
    else:
        current_kernel = run_cmd(
            test_instance, "sudo grubby --default-kernel", expect_ret=0
        ).strip("\n")
        if "el7" in kernel_version:
        ...
        run_cmd(
            test_instance,
            "sudo grubby --info=%s" % default_kernel,
            expect_ret=0,
            msg="check if default kernel is installed",
        )
        ...
        run_cmd(
            ...
            cmd,
            expect_ret=0,
            msg="Set default boot kernel to default kernel",
        )
        if "kmemleak=on" in cmdline:
            cmd = 'sudo grubby --update-kernel=ALL --remove-args="kmemleak=on"'
            run_cmd(
    ...
    run_cmd(
        test_instance, "sudo reboot", msg="reboot OS to take default kernel effects"
    )
    time.sleep(60)
    init_connection(test_instance, timeout=test_instance.ssh_timeout)
...
def collect_kmemleak(test_instance=None):
    ...
    if "debug" not in out or "kmemleak=on" not in out:
        test_instance.log.info(
            "collect_kmemleak requires debug kernel with kmemleak=on"
        )
        return False
...
def get_active_nic(test_instance=None, rmt_node=None, vm=None):
    ...
    for net in output.split("\n"):
        if len(net) < 3:
            continue
        cmd = "sudo ping {} -c 6 -I {}".format(
            test_instance.params.get("ping_server"), net
        )
...
def imds_tracer_tool(
    ...
    is_pkg_installed(
        test_instance, pkg_name="bcc-tools", cancel_case=is_return, timeout=600
    )
    is_pkg_installed(
        test_instance, pkg_name="libbpf", cancel_case=is_return, timeout=600
    )
    ...
    ret = run_cmd(
        test_instance, bpf_test_cmd, msg="check if bcc-tools work", ret_status=True
    )
    if ret != 124:
        if is_return:
    ...
    ret = run_cmd(
        test_instance, "systemctl status imds_tracer_tool.service", ret_status=True
    )
    if ret != 0:
        if cleanup:
            test_instance.log.info(
                "imds_tracer_tool service is not enabled, no need to cleanup"
            )
            return True
        ...
        run_cmd(
            test_instance,
            "cd aws-imds-packet-analyzer; sudo ./activate-tracer-service.sh",
        )
        time.sleep(30)
        ret = run_cmd(
            test_instance, "systemctl status imds_tracer_tool.service", ret_status=True
    ...
    if log_check:
        ...
        while True:
            ...
            check_log(
                test_instance, "IMDSv1", log_cmd="sudo cat /var/log/imds/imds-trace.log"
            )
            time_end = int(time.time())
    ...
    if cleanup:
        ...
        run_cmd(
            test_instance,
            "cd aws-imds-packet-analyzer; sudo ./deactivate-tracer-service.sh",
        )
...
def configure_repo(
    test_instance, repo_type=None, repo_url_param=None, rmt_node=None, vm=None
):
    """
...
sslverify=0
        ...
        fh, tmp_repo_file = tempfile.mkstemp(
            suffix="_rhel.repo", dir="/tmp", text=False
        )
        id = 0
        with open(tmp_repo_file, "a") as fh:
            for repo_url in repo_url_param.split(","):
                repo_str = repo_temp.substitute(
                    repo_type=repo_type, id=id, repo_url=repo_url
                )
                test_instance.log.info(
                    "Add new repo %s to %s" % (repo_url, tmp_repo_file)
                )
...
def determine_architecture(test_instance):
