...
class EC2VM(VMResource):
    ...
    def __init__(self, params, vendor="redhat"):
        ...
        if self.profile_name is None:
            LOG.info(
                "No profile_name provided, leave boto3 to detect authentication methods"
            )
        ...
        self.client = self.session.client(
            "ec2", config=config, region_name=params.get("region")
        )
        ...
        LOG.info(
            "AMI picked vendor:{} ami:{} key:{}".format(
                vendor, self.ami_id, self.ssh_key_name
            )
        )
    ...
    def create(
        ...
        try:
            ...
            LOG.info(
                "Hibernation supported status: {}".format(self.hibernation_support)
            )
        except Exception as error:
            LOG.info(
                "Cannot determin Hibernation status, disable in launch:{}".format(error)
            )
        try:
        ...
        try:
            self.root_device_name = self.client.describe_images(ImageIds=[self.ami_id])[
                "Images"
            ][0]["RootDeviceName"]
            LOG.info("root device name: {}".format(self.root_device_name))
        except Exception as error:
            LOG.info(
                "Cannot determin root device name, use default {}".format(
        ...
        if enable_hibernation and self.volume_size < 50:
            ...
            self.volume_size = 50
            LOG.info(
                "hibernation_support enabled, change volume size to {}".format(
                    self.volume_size
                )
            )
        if enable_enclave and self.volume_size < 50:
            LOG.info(
                "create with enclave enabled, change volume size to {}".format(
                    self.volume_size
                )
            )
            self.volume_size = 50
            ...
            "TagSpecifications": [
                {
                    ...
                    "Tags": [
                        {"Key": "Name", "Value": self.tag},
                    ],
                },
            ],
            "HibernationOptions": {"Configured": enable_hibernation},
            "EnclaveOptions": {"Enabled": enable_enclave},
            "MetadataOptions": {
        ...
        if self.placement_group_name:
            ...
            if any([i in self.instance_type for i in not_support_placement]):
                LOG.info(
                    "placement groups are not supported by the {} instance type.".format(
                        not_support_placement
                    )
                )
            else:
        ...
        if enable_ipv6only:
            ...
            if not subnet.ipv6_native:
                self.msgs = (
                    "subnet_id_ipv6only {} is not a real ipv6 only subnet".format(
                        self.subnet_id_ipv6only
                    )
                )
                LOG.info(self.msgs)
                return False
        ...
        if self.additionalinfo:
            for additionalinfo in self.additionalinfo.split(";"):
                ...
                for volume_size in [10, 20, 40, 50]:
                    LOG.info("Create instance {}".format(vm_kwargs))
                    try:
                        self.ec2_instance = self.resource.create_instances(**vm_kwargs)[
                            0
                        ]
                        self.is_created = True
                        break
                        ...
                        if "UnsupportedHibernationConfiguration" in str(err):
                            vm_kwargs["BlockDeviceMappings"][0]["Ebs"][
                                "VolumeSize"
                            ] = volume_size
                            ...
                            if volume_size == 40:
                                ...
                                self.hibernation_support = False
                                vm_kwargs["HibernationOptions"][
                                    "Configured"
                                ] = self.hibernation_support
                            continue
                if self.is_created:
                    break
    ...
    def private_ip(self):
        self.ec2_instance.reload()
        LOG.info(
            "instance: {} private_ip_address is: {}".format(
                self.id, self.ec2_instance.private_ip_address
            )
        )
    ...
    def disk_count(self):
        ...
        default_disks = 1
        instance_types_dict = self.client.describe_instance_types(
            InstanceTypes=[self.instance_type]
        )
        ...
        for instance_type in instance_types:
            if instance_type["InstanceStorageSupported"]:
                default_disks = (
                    instance_type["InstanceStorageInfo"]["Disks"][0]["Count"] + 1
                )
    ...
    def net_bandwidth(self):
        ...
        if not instance_types:
            LOG.info(
                "Cannot get instance type info, return default bandwidth {}".format(
                    default_bandwidth
                )
            )
        ...
        if "Gigabit" in net_bandwidth:
            ...
            LOG.info(
                "{} net setting spec {}, ret {}".format(
                    self.instance_type, net_bandwidth, default_bandwidth
                )
            )
            return default_bandwidth
    ...
    def is_uefi_boot(self):
        ...
        try:
            ...
            LOG.info(
                "boot mode set: {} current:".format(bootmode_set, boot_mode_current)
            )
    ...
    def start(self, wait=True):
        ...
        if self.additionalinfo != None and self.additionalinfo != "":
            for additionalinfo in self.additionalinfo.split(";"):
                try:
                    LOG.info(
                        "Start instance using AdditionalInfo: {}".format(additionalinfo)
                    )
                    ...
                    LOG.error(
                        "Failed to start instance, try another AdditionalInfo: {}".format(
                            err
                        )
                    )
        if not start_ok:
    ...
    def stop(self, wait=True, loops=4, hibernate=False):
        try:
            LOG.info(
                "Stopping instance {} with hibernation {}".format(self.id, hibernate)
            )
    ...
    def send_nmi(self):
        try:
            LOG.info("Send diagnostic interrupt to %s" % (self.ec2_instance.id))
            ...
            LOG.error(
                "Failed to send_diagnostic_interrupt to {}:{}".format(
                    self.ec2_instance.id, err
                )
            )
    ...
    def get_console_log(self, silent=False):
        ...
        try:
            ...
            LOG.error(
                "Failed to get console log, try without latest option! {}".format(err)
            )
        try:
    ...
    def attach_block(self, disk, target, wait=True, timeout=120):
        try:
            ...
            self.ec2_instance.attach_volume(
                Device=target, VolumeId=disk.id, DryRun=False
            )
        except Exception as err:
            LOG.error(err)
            return False
    ...
    def assign_new_ip(self):
        ...
        try:
            ret = nic.assign_private_ip_addresses(
                AllowReassignment=True, SecondaryPrivateIpAddressCount=1
            )
    ...
    def assign_secondary_ips(self, secondary_ip_count=1):
        ...
        for i in range(0, secondary_ip_count):
            ...
            if new_ip:
                ...
                LOG.info(
                    "Cannot add more, current secondary list:{}".format(
                        self.secondary_ip_list
                    )
                )
                break
    ...
    def create(self, wait=True):
        ...
        try:
            ...
            if self.type == "sc1" and self.size < 500:
                self.size = 500
                LOG.info(
                    "{} minimal size is 500G, create 500G disk instead.".format(
                        self.type
                    )
                )
            disk_kwargs = {
    ...
    def modify_disk_size(self, os_disk_size=10, expand_num=10):
        ...
        try:
            response = self.client.modify_volume(
                VolumeId=self.id, Size=os_disk_size + expand_num
            )
            time.sleep(20)
            self.get_state()
        except Exception as err:
...
class EC2NIC(NetworkResource):
    ...
    def is_free(self):
        ...
        if self.__network_interface.status == "in-use":
            LOG.info("%s network interface is in use.", self.__network_interface.id)
            return True
        return False
    ...
    def add_tag(self):
        try:
            self.__network_interface.reload()
            self.__network_interface.create_tags(
                Tags=[
                    {"Key": "Name", "Value": self.tag},
                ]
            )
    ...
    def create(self, interfaceType="interface"):
        ...
        try:
            if interfaceType == "efa":
                ...
                LOG.info(
                    "%s efa network interface created!" % self.__network_interface.id
                )
                self.add_tag()
                ...
                LOG.info("%s network interface created!" % self.__network_interface.id)
                self.add_tag()
                self.id = self.__network_interface.id
                return True
        except Exception as err:
    ...
    def attach_to_instance(self, instance_id, device_index, wait=True):
        ...
        try:
            LOG.info(
                "Try to attach %s to %s" % (self.__network_interface.id, instance_id)
            )
    ...
    def detach_from_instance(self, instance_id, wait=True, force=False):
        ...
        try:
            LOG.info(
                "Try to detach %s from %s" % (self.__network_interface.id, instance_id)
            )
            self.__network_interface.reload()
            ...
            LOG.error("NIC cannot detach from %s error %s" % (instance_id, err))
            return False
        ...
        try:
            ...
            for ip_permission in ip_permissions:
                if ip_permission["FromPort"] == port:
                    LOG.info(
                        f"Port {port} has existed in the inbound ruel,no need to add."
                    )
                    is_port_exist = True
    ...
    def remove_inbound_rule(self, instance_id, port):
        ...
        try:
            ...
            security_group_id = instance.security_groups[0]["GroupId"]
            LOG.info(
                f"Trying to remove port {port} from security group {security_group_id}"
            )
