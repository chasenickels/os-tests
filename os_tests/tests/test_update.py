...
class TestUpgrade(unittest.TestCase):
    ...
    def _confirm_answer_file(self):
        ...
        answer_file = utils_lib.run_cmd(
            self, cmd, expect_ret=0, msg="cat /var/log/leapp/answerfile"
        )
        p = re.compile(r"[\[](.*?)[]]", re.S)
    ...
    def _config_PermitRootLogin(self):
        ...
        if utils_lib.is_aws(self):
            ...
            utils_lib.run_cmd(
                ...
                'sudo bash -c "{}"'.format(cmd),
                expect_ret=0,
                msg="Edit %s to avoid inhibitor about remote login using root account"
                % sshd_config,
            )
    ...
    def _remove_driver(self):
        output = utils_lib.run_cmd(
            self, "sudo lsmod", expect_ret=0, msg="check loaded drivers"
        )
        for line in output.splitlines():
    ...
    def _remove_package(self):
        ...
        ret = utils_lib.run_cmd(
            self, cmd, ret_status=True, msg="Check if there is kernel-devel package"
        )
        if ret == 0:
            utils_lib.run_cmd(
                self,
    ...
    def test_dnf_update(self):
        ...
        if dnf_repo_url:
            utils_lib.configure_repo(
                self, repo_type="dnf_repo", repo_url_param=dnf_repo_url
            )
            # prepare dnf update
            ...
            utils_lib.run_cmd(
                self,
                "sudo yum remove -y kernel-debug-core kernel-debug-modules",
                expect_ret=0,
            )
            utils_lib.run_cmd(self, "sudo yum repolist enabled", expect_ret=0)
            utils_lib.run_cmd(
                self, 'sudo bash -c "echo "" > /var/log/secure"', expect_ret=0
            )
            utils_lib.run_cmd(
                self, "sudo  rm -rf /var/log/cloud-init.log", expect_ret=0
            )
            utils_lib.run_cmd(
                self, "sudo  rm -rf /var/log/cloud-init-output.log", expect_ret=0
            )
            utils_lib.run_cmd(
                self, 'sudo bash -c "echo "minrate=200" >> /etc/yum.conf"', expect_ret=0
            )
            utils_lib.run_cmd(
                self,
                'sudo bash -c "echo "timeout=1800" >> /etc/yum.conf"',
                expect_ret=0,
            )

        # Prepare rhsm for rhsm update
        if self.params.get("subscription_username") and self.params.get(
        ...
        if ret_val != 0:
            ...
            utils_lib.run_cmd(
                self, "sudo reboot", msg="Reboot system to the latest kernel"
            )
    ...
    def test_leapp_upgrade_rhui(self):
        ...
        ret = utils_lib.run_cmd(
            self, cmd, ret_status=True, msg="Check if there is rhui client in system"
        )
        if ret != 0:
        ...
        if x_version == 7:
            ...
            utils_lib.run_cmd(
                ...
                cmd,
                expect_ret=0,
                msg="Enable repos for leapp-rhui-{} package installation".format(
                    platform
                ),
            )
        ...
        utils_lib.is_pkg_installed(
            self,
            pkg_name="leapp-rhui-{}".format(platform),
            is_install=True,
            cancel_case=True,
        )
        utils_lib.is_pkg_installed(
            self, pkg_name="leapp", is_install=True, cancel_case=True
        )
        utils_lib.is_pkg_installed(
            self, pkg_name="leapp-upgrade", is_install=True, cancel_case=True
        )
        # Prepare for upgrade
        self._remove_driver()
        ...
        if target_version:
            cmd_preupgrade = (
                "sudo LEAPP_UNSUPPORTED=1 LEAPP_DEVEL_TARGET_RELEASE=%s leapp preupgrade --debug --no-rhsm"
                % (target_version)
            )
            cmd_upgrade = (
                "sudo LEAPP_UNSUPPORTED=1 LEAPP_DEVEL_TARGET_RELEASE=%s leapp upgrade --debug --no-rhsm"
                % (target_version)
            )
        else:
        ...
        if ret != 0:
            ...
            if ret != 0:
                ...
                utils_lib.run_cmd(
                    self, "sudo reboot", msg="Reboot system after leapp upgrade"
                )
                utils_lib.run_cmd(
                    self, "ls /boot/grub2", msg="Check grub2 configure files"
                )
                time.sleep(600)
                utils_lib.init_connection(self, timeout=self.ssh_timeout)
                utils_lib.collect_basic_info(self)
                ...
                if x_version_upgrade != x_version + 1:
                    self.FailTest(
                        "Leapp upgrade failed since did not upgrade to target release"
                    )
    ...
    def test_leapp_upgrade_customrepo(self):
        ...
        if leapp_target_repo_url:
            utils_lib.configure_repo(
                self,
                repo_type="leapp_target_repo",
                repo_url_param=leapp_target_repo_url,
            )
        # Peform leapp preupgrade via custom repo
        target_version = self.params.get("target_version")
        if target_version:
            cmd_preupgrade = "sudo leapp preupgrade --debug --no-rhsm --target %s" % (
                target_version
            )
            cmd_upgrade = "sudo leapp upgrade --debug --no-rhsm --target %s" % (
                target_version
            )
        else:
            cmd_preupgrade = "sudo leapp preupgrade --debug --no-rhsm"
        ...
        if ret != 0:
            ...
            for i in range(1, 10):
                ...
                if ret_val > 0:
                    self.log.info(
                        "Failed to upgrade system, try again! max:20 now:%s" % i
                    )
                    ret_val = utils_lib.run_cmd(
                        self,
                        cmd_upgrade,
    ...
    def test_leapp_upgrade_rhsm(self):
        ...
        if target_version:
            cmd_preupgrade = "sudo leapp preupgrade --debug --target %s" % (
                target_version
            )
    ...
    def test_pkg_install(self):
        ...
        if pkg_repo_url:
            utils_lib.configure_repo(
                self, repo_type="pkg_repo", repo_url_param=pkg_repo_url
            )
        ...
        utils_lib.run_cmd(
            self, "sudo rm -rf {}".format(pkg_repo), msg="delete pkg_repo"
        )
