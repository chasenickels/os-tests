...
from os_tests.libs.ec2 import EC2InstanceManager
...
class TestUbuntu(unittest.TestCase):
    ...
    def test_run_ubuntu_qual(self):
        ...
        utils_lib.run_cmd(
            self,
            'sudo bash -c "{}"'.format(ubuntu_qual_run),
            timeout=500,
            msg="the system might loss connection if the script cannot finish normally.",
        )
